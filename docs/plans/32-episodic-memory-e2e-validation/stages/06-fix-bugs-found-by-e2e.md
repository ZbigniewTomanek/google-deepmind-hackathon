...
| B5 | **Extraction `ModelAPIError: Connection error`**: `extract_episode` jobs fail with connection errors to the Gemini API; `route_episode` jobs succeed | Blocker for Stage 4+ | **NOT FIXED** | Likely API key / network issue; needs investigation |
...
After all fixes, the test output shows:
...
- **Stage 4**: FAILS — extraction jobs fail with `ModelAPIError: Connection error` (Gemini API unreachable from extraction worker). `route_episode` jobs succeed, `extract_episode` jobs fail after 4 retries.
- **Stage 5**: Not reached (depends on Stage 4)
...
| `src/neocortex/db/adapter.py` | Episode over-fetch, nucleus-only `seen_episode_ids`, neighbor-preserving truncation in `recall()`, temporary debug logging (to remove) | Done (debug to remove) |
...
- [ ] Extraction connection error resolved (B5)
- [ ] Debug logging removed from `adapter.py`
- [ ] Full E2E test passes: `./scripts/run_e2e.sh scripts/e2e_episodic_memory_test.py`
- [ ] Unit tests pass (875+)
- [ ] No regressions in other E2E scripts (optional: `./scripts/run_e2e.sh scripts/e2e_cognitive_recall_test.py`)
