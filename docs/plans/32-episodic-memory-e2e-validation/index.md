...
| 6 | [Fix bugs found by E2E](stages/06-fix-bugs-found-by-e2e.md) | DONE | Fixed B1-B5. B5 was missing OPENAI_API_KEY in .env (extraction uses openai-responses model). Also increased job timeout to 600s, added failed-job tracking, raised combined-recall limit to 50. All 5 stages pass. | fix: resolve extraction API errors and finalize E2E episodic memory test |
...
**I2 — Extraction API connection errors (Stage 6, B5)**: RESOLVED. `extract_episode` jobs failed with `pydantic_ai.exceptions.ModelAPIError: Connection error` because the extraction pipeline uses `openai-responses:gpt-5.4-mini` (not Gemini), and `OPENAI_API_KEY` was missing from `.env`. The DNS resolution error (`[Errno 8] nodename nor servname provided, or not known`) was the OpenAI SDK failing to connect without valid credentials. Fixed by adding `OPENAI_API_KEY` to `.env`. Job timeout also increased from 120s to 600s because 37 extraction+routing jobs take ~450s with 4 workers.
